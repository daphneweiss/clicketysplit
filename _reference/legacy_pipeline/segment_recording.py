...
def _refine_boundary(times, energy, coarse_t, direction='start', window_ms=40):
    ...
    if direction == 'start':
        ...
        return coarse_t  # no clear onset found — don't expand into silence
    else:
        ...
        return coarse_t  # no clear offset found — don't expand into silence
...
def detect_segments_silero(audio, sr, vad_threshold=0.5):
    """
...
def classify_and_label(segments, stimuli):
    ...
    intro_end_idx = 0
    if len(gaps) >= 2:
        # Limit scan to the first ~8 segments — intro is a few utterances at most.
        # n_seg // 3 was too large for spliced multi-word recordings where a big
        # gap mid-recording (splice point) would absorb all preceding real words.
...
def process_single(audio_path, stimlist_path, output_dir, audio_label=None,
    ...
    stimuli = load_stimulus_list(stimlist_path) if stimlist_path else None
    if stimuli:
    ...
    kwargs = {"vad_threshold": vad_threshold} if detect_fn is detect_segments_silero else {}
    segments, analysis = detect_fn(audio, sr, **kwargs)
...
def process_multiple(audio_paths, stimlist_path, output_dir, audio_label=None,
                     ...
                     vad_method='silero', vad_threshold=0.5):
    """
