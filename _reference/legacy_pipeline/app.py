...
def api_audio_file(speaker, condition, filename):
    ...
    safe_path = RECORDINGS_DIR / speaker / condition / Path(filename).name
...
def api_detect():
    ...
    # Re-running detection supersedes any previous review — remove stale reviewed file
    stale = EXPERIMENT_DIR / speaker / condition / "reviewed_segments.json"
    if stale.exists():
        stale.unlink()
...
def api_export():
    ...
    # Renumber token_index per word sequentially (must match frontend logic)
    ...
    for seg in segments:
        if seg.get("segment_type") == "word":
            ...
            seg["cluster_size"] = 0  # filled below
    for seg in segments:
        if seg.get("segment_type") == "word":
            seg["cluster_size"] = word_counter_renum.get(seg.get("assigned_name", ""), 1)

    # If selected_tokens is provided, filter segments
    if selected_tokens:
        filtered = []
        for seg in segments:
            if seg.get("segment_type") != "word":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TotalRecal Applet")
    parser.add_argument("--port", type=int, default=5000)
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--fresh", action="store_true",
                        help="Start fresh — delete autosave so no old session is restored")
    args = parser.parse_args()

    if args.fresh:
        autosave = SESSIONS_DIR / "_autosave.pkl"
        if autosave.exists():
            autosave.unlink()
            print("  (Deleted _autosave.pkl — starting fresh)")
